...
<body>
    ...
    <div class="modal fade" id="pointModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel"><i class="fas fa-edit"></i> Edit Point</h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form action="<?= base_url('leafletdraw/simpan_edit_data_point') ?>" method="POST">
                        <?= csrf_field() ?>

                        <input type="hidden" name="id_point" id="id_point">

                        <label for="edit_point_name">Nama</label>
                        <input type="text" class="form-control" name="edit_point_name" id="edit_point_name">

                        <label for="edit_point_deskripsi">Deskripsi</label>
                        <input type="text" class="form-control" name="edit_point_deskripsi" id="edit_point_deskripsi">
    ...
    <script>
        ...
        var point = L.geoJson(null, {
            ...
            onEachFeature: function(feature, layer) {
                ...
                layer.on({
                    click: function(e) {
                        //Open Modal
                        $('#id_point').val(feature.properties.id);
                        $('#edit_point_name').val(feature.properties.nama);
                        $('#edit_point_deskripsi').val(feature.properties.deskripsi);
        ...
        $.getJSON("<?= base_url('api/one_point') . "/" . $idpoint ?>", function(data) {
            point.addData(data);
            map.addLayer(point);
        ...
        map.on(L.Draw.Event.EDITED, function(e) {
            ...
            layers.eachLayer(function(layer) {
                ...
                var confirm = window.confirm("Apakah Anda yakin akan mengubah point ini?");
                if (confirm) {
                    // Hapus pointdengan ajax
                    $.ajax({
                        url: "<?= base_url('leafletdraw/simpan_edit_geom_point') ?>",
                        type: "POST",
                        data: {
                            "id": <?= $idpoint ?>,
                            "geom": drawnItemWKT,
